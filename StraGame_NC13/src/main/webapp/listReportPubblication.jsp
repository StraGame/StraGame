<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
   ...
   <% ArrayList<ReportPubblicationBean> lista = (ArrayList<ReportPubblicationBean>) request.getAttribute("lista"); %>

	<% ArrayList<NewsBean> news = (ArrayList<NewsBean>) request.getAttribute("news"); %>
	
	<% ArrayList<PubblicationBean> pubblicazioni = (ArrayList<PubblicationBean>) request.getAttribute("pubblicazioni");%>
<!DOCTYPE html>
<html lang="it">

<%@ page contentType="text/html; charset=ISO-8859-1" import="java.util.*, java.util.ArrayList, model.ReportPubblicationBean , model.NewsBean, model.PubblicationBean"%>

<% if(news==null || lista==null || pubblicazioni==null){
	response.sendRedirect("./ListReportPubblicationServlet");	
	return;
}%>
    ...
    <section class="header-descriptin329">
        <div class="container">
            <ol class="breadcrumb breadcrumb839">
                <li><a href="index.jsp">Home</a></li>
                <li class="active">Pubblicazioni Segnalate</li>
            </ol>
        </div>
    </section>
<!-- ======content section/body=====-->
    <section class="main-content920">
        <div class="container">
            <p class='titolo'>Pubblicazioni Segnalate</p>
            <div class="row">
                <div class="col-md-9">
                    <div id="main">
                    <%for(ReportPubblicationBean report : lista){ %>
                    	<%for(PubblicationBean bean : pubblicazioni) { %>
                    		<% if(report.getCodicePubblicazione()==bean.getCodicePubblicazione()) { %>
	                        <div class="question-type2033">
	                            <div class="row">
	                                <div class="col-md-1">
	                                    <div class="left-user12923 left-user12923-repeat">
	                                        <img src="GetPhoto?username=<%=report.getAutore()%>&action=user" onerror="this.src='image/images.png'" alt="image">
	                                    </div>
	                                </div>
	                                <div class="col-md-9">
	                                    ...
	                                    <div class="col-md-2">
	                                        <div class="ques-type302">
	                                            <a href="ReportDetailsServlet?id=<%=report.getCodicePubblicazione() %>&autore=<%=report.getAutore() %>" ><button class="button viewSegnalazionep">Vedi segnalazione</button></a>
	                                            <a href="ListReportPubblicationServlet?action=DeleteReport&id=<%=report.getCodicePubblicazione() %>&autore=<%=report.getAutore() %>" ><button class="button approve">Approva Pubblicazione</button></a>
	                                            <a href="ListReportPubblicationServlet?action=DeletePubblication&id=<%=report.getCodicePubblicazione() %>&autore=<%=report.getAutore() %>" ><button class="button delete">Elimina Pubblicazione</button></a>
	                                        </div>
	                                    </div>
	                                </div>
	                            </div> 
